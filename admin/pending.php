<?php
// admin/pending.php - DARK THEME REDESIGN
...
$status_filter = 'reported'; // Force pending status
...
// Build query
$where = ["i.status = 'reported'"]; // Only pending issues
$params = [];
$types = "";
...
// Get pending issues with details
$sql = "SELECT i.*, 
...
// Get priority counts for pending issues
$priority_counts = [
    'urgent' => 0,
    'high' => 0,
    'medium' => 0,
...
$aging_result = $db->query("
    SELECT 
        ...
        SUM(CASE WHEN created_at < DATE_SUB(NOW(), INTERVAL 7 DAY) THEN 1 ELSE 0 END) as over_week,
        SUM(CASE WHEN created_at < DATE_SUB(NOW(), INTERVAL 30 DAY) THEN 1 ELSE 0 END) as over_month
    FROM issues 
    WHERE status = 'reported'
");
if ($aging_result) {
    $aging_stats = $aging_result->fetch_assoc();
}
...
<head>
    ...
    <title>Pending Issues - Admin Panel - Civic Issue Portal</title>
    ...
    <style>
        ...
        * {
        ...
        .stat-card::before {
            ...
            right: 0;
            height: 4px;
            background: linear-gradient(135deg, var(--warning), var(--danger), var(--gold));
...
<body>
    ...
    <div class="admin-wrapper">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <div class="sidebar-menu">
                ...
                <a href="pending.php" class="menu-item active">
        ...
        <div class="main-content">
            ...
            <div class="top-bar">
                <div class="page-title">
                    <h1><i class="fas fa-clock" style="color: var(--warning);"></i> Pending Issues</h1>
                    <p>Issues waiting to be reviewed and assigned</p>
                </div>
            ...
            <div class="stats-grid">
                <div class="stat-card" onclick="window.location.href='pending.php'">
                    ...
                    <div class="stat-trend">
                        <i class="fas fa-exclamation-circle"></i> Awaiting action
                    </div>
                </div>
                ...
                <div class="stat-card" onclick="window.location.href='?priority=urgent'">
                    ...
                    <div class="stat-trend">
                        <i class="fas fa-exclamation-circle" style="color: var(--danger);"></i> Immediate attention
                    </div>
                </div>

                <div class="stat-card" onclick="window.location.href='?assigned=unassigned'">
                    <div class="stat-header">
                        <span class="stat-title">Over 7 Days</span>
                        <div class="stat-icon" style="color: var(--danger);">
                            <i class="fas fa-calendar-times"></i>
                        </div>
                    </div>
                    <div class="stat-value"><?php echo $aging_stats['over_week']; ?></div>
                    <div class="stat-trend">
                        <i class="fas fa-exclamation-circle" style="color: var(--danger);"></i> Aging issues
                    </div>
                </div>

                <div class="stat-card" onclick="window.location.href='?date_from=<?php echo date('Y-m-d'); ?>'">
                    <div class="stat-header">
                        <span class="stat-title">Reported Today</span>
                        <div class="stat-icon" style="color: var(--success);">
                            <i class="fas fa-calendar-day"></i>
                        </div>
                    </div>
                    <div class="stat-value"><?php echo $aging_stats['today']; ?></div>
                    <div class="stat-trend">
                        <i class="fas fa-arrow-up" style="color: var(--success);"></i> New today
                    </div>
                </div>
            </div>
            ...
            <div class="filter-section">
                <div class="filter-header">
                    <div class="filter-title">
                        <i class="fas fa-filter"></i> Filter Pending Issues
                    </div>
                    <a href="pending.php" class="btn btn-secondary btn-sm">
            ...
            <div class="table-section">
                <div class="section-header">
                    <h2><i class="fas fa-clock"></i> Pending Issues List</h2>
                        ...
                        <tbody>
                            <?php if ($issues && $issues->num_rows > 0): ?>
                                <?php while($issue = $issues->fetch_assoc()): 
                                    $age_days = floor((time() - strtotime($issue['created_at'])) / 86400);
                                    $age_class = $age_days > 7 ? 'aging-critical' : ($age_days > 3 ? 'aging-warning' : 'aging-normal');
                                ?>
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            <span class="aging-badge <?php echo $age_class; ?>">
                                                <i class="fas fa-clock"></i> <?php echo $age_days; ?>d
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                <button class="action-btn assign" onclick="openAssignModal(<?php echo $issue['issue_id']; ?>, '<?php echo addslashes($issue['title']); ?>', '<?php echo $issue['assigned_to']; ?>')" title="Assign To">
                                ...
                                <tr>
                                    <td colspan="8" style="text-align: center; padding: 3rem;">
                                        <div class="empty-state">
                                            ...
                                            <p>All caught up! No issues waiting for review.</p>
                                            <a href="../report-issue.php" class="btn btn-primary" style="margin-top: 1rem;">
    ...
    <div id="assignModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3><i class="fas fa-user-tag"></i> Assign Issue</h3>
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    <label><i class="fas fa-comment"></i> Remarks (Optional)</label>
                    <textarea name="remarks" class="form-control" rows="3" placeholder="Add assignment notes..."></textarea>
                </div>
    ...
    <script>
        ...
        function openStatusModal(issueId, currentStatus, issueTitle, assignedTo) {
            ...
            statusSelect.value = 'in_progress'; // Default to in_progress for pending issues
        ...
        function exportToCSV() {
            ...
            a.href = url;
            a.download = 'pending_issues_' + new Date().toISOString().split('T')[0] + '.csv';
            a.click();
        ...
        const cards = document.querySelectorAll('.stat-card, .action-btn, .menu-item');
        cards.forEach(card => {
